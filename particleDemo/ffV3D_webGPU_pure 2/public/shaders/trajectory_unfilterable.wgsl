...
struct UniformBlock {
    ...
    u_mercatorCenterHigh: vec2f,
    u_mercatorCenterLow: vec2f,
    u_matrix: mat4x4f
}
...
fn translateRelativeToEye(high: vec2f, low: vec2f) -> vec2f {
    let highDiff = high - flowUniform.u_mercatorCenterHigh;
    let lowDiff = low - flowUniform.u_mercatorCenterLow;

    return highDiff + lowDiff;
}

fn sampleGeoPosition(pos: vec2f, transformTexture: texture_2d<f32>) -> vec2f {

    let textureSize = textureDimensions(transformTexture, 0);
    let uv: vec2f = pos * vec2f(textureSize);
    // let parity = sign(uv - vec2f(uv0) - vec2f(0.5));
    var f = fract(uv);
...
fn ReCoordinate(pos: vec2f) -> vec4f {
    ...
    // let geoPos0 = textureLoad(transformHighTexture, uv0, 0).xy;
    // let geoPos1 = textureLoad(transformHighTexture, uv1, 0).xy;
    // let geoPos2 = textureLoad(transformHighTexture, uv2, 0).xy;
    // let geoPos3 = textureLoad(transformHighTexture, uv3, 0).xy;

    // let lerp = abs(f - vec2f(0.5));
    // let geoPos = mix(mix(geoPos0.xy, geoPos1.xy, lerp.x), mix(geoPos2, geoPos3, lerp.x), lerp.y);
    ...
    let geoPos = translateRelativeToEye(highGeoPos, vec2f(0.0));
