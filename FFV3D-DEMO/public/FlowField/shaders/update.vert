...
{   
    ...
    // vec2 seed = particleInfo.xy;
    float drop_rate = dropRate + velocity * dropRateBump; //速度越大，越容易重置
...
void simulation(vec2 resolution)
{
    ...
    //要注意这个/resolution，本身整个canvas就是在(0,1)的，如果不/resolution
    //那量纲必然不匹配，速度极大，xy一次就移动好远，一段就死，但方向正确，不会是45°情况
    vec2 nPos = vec2(particleInfo.xy + speed * speedFactor / resolution);//移动后位置
    nPos = clamp(nPos, vec2(0.0), vec2(1.0));//保01
