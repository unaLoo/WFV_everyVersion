...
@group(0) @binding(6) var<storage,read_write> particlePosition:array<f32>;//(x,y,z)  ---- segNum*particleNum*3
@group(0) @binding(7) var<storage,read_write> particleAttribute:array<f32>;//speedRate -- segNum*particleNum
...
fn simulation_2(downTex:texture_2d<f32>, pIndex:u32,npIndex:u32)
{
    //基于nowpos 速度信息 来计算nextpos，nextAttrib
    let pos = vec3f(particlePosition[pIndex*3],particlePosition[pIndex*3+1],particlePosition[pIndex*3+2]);
    ...
    // let progress    = mod(pos.z,ublock1.deltaHeight)/ublock1.deltaHeight;
    let progress = pos.z - ublock1.deltaHeight * floor(pos.z/ublock1.deltaHeight);
    let speed  = mix(getSpeed(pos.xy,downTex),0.0,progress);
    var delta_zvalue= speed*zvalue_unit;
    // delta_zvalue = mix(0.00000,zvalue_unit,delta_zvalue);
...
fn cMain(@builtin(global_invocation_id) id:vec3<u32>){
    let indexOfs = u32(ublock1.startReadIndex*ublock1.particleNum);
    let nextOfs  = u32(ublock1.startStorageIndex*ublock1.particleNum);
    let particleIndex = indexOfs + id.y * ublock1.groupSize.x * blockSize + id.x;
    let nxtParticleIndex = nextOfs+id.y * ublock1.groupSize.x * blockSize + id.x;
    ...
    if(nowPos.z<ublock1.deltaHeight){
    ...
    }else if(nowPos.z<3*ublock1.deltaHeight) {

        simulation_2(fakeTexture_hi,particleIndex,nxtParticleIndex);
        ...
        particlePosition[nxtParticleIndex*3+2]=3*ublock1.deltaHeight;
