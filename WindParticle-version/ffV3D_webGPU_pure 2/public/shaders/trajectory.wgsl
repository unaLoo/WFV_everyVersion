...
@group(2) @binding(3) var<storage> positionTest: array<f32>;
...
fn ReCoordinate(pos: vec2f, ads: u32) -> vec4f {
    ...
    let geoPos_z = data_v[ ads * 3 + 2];

    let res = flowUniform.u_matrix * vec4f(geoPos , geoPos_z , 1.0);
    // let res = flowUniform.u_matrix * vec4f(geoPos, 0.001, 1.0);
    // let res = flowUniform.u_matrix * vec4f(geoPos, positionTest[ads], 1.0);

    return res;
}

fn transfer_to_clip_space(pos: vec2f , ads: u32) -> vec4f {
    
    return ReCoordinate(pos, ads);
}

fn get_clip_position(address: u32 , ads: u32) -> vec4f {

    return transfer_to_clip_space(vec2f(data_v[3 * address], data_v[3 * address + 1]), ads);
}
...
fn vMain(vsInput: VertexInput) -> VertexOutput {
    ...
    let cn_pos_CS = get_clip_position(c_address, c_address);
    let nn_pos_CS = get_clip_position(n_address, n_address);
